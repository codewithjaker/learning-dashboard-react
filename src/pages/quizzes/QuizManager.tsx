    ...
    <div className="container max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
          {currentQuiz.id && (
            <Button variant="outline" onClick={() => navigate(`/quiz/take/${currentQuiz.id}`)}>
              Take Quiz
            </Button>
          )}
...
import { useDispatch, useSelector } from 'react-redux';
import { type RootState } from '../../store';
import { fetchQuiz, updateQuiz, deleteQuiz, fetchAttemptHistory } from '../../store/slices/quizSlice';
import { QuizForm } from './QuizForm';
import { Button } from '../../components/ui/button';
import { Card, CardContent } from '../../components/ui/card';
import { useToast } from '../../components/ui/use-toast';
...
export default function QuizManager() {
  ...
  const dispatch = useDispatch();
  const { toast } = useToast();
  const { currentQuiz, isLoading, attemptHistory } = useSelector((state: RootState) => state.quiz);
  const [isEditing, setIsEditing] = useState(false);

  useEffect(() => {
    if (quizId && !currentQuiz) {
      dispatch(fetchQuiz(parseInt(quizId)));
    }
  }, [dispatch, quizId, currentQuiz]);

  useEffect(() => {
    if (currentQuiz?.id) {
      dispatch(fetchAttemptHistory(currentQuiz.id));
    }
  ...
  if (isEditing) {
    return <QuizForm initialData={currentQuiz} onSubmit={handleUpdate} onCancel={() => setIsEditing(false)} />;
  }

  const hasAttempts = attemptHistory && attemptHistory.length > 0;
  const remainingAttempts = currentQuiz.maxAttempts - (attemptHistory?.length || 0);
  const canTake = remainingAttempts > 0 || currentQuiz.maxAttempts === 0;
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
          {currentQuiz.id && (
            <Button
              variant="outline"
              onClick={() => navigate(`/quiz/take/${currentQuiz.id}`)}
              disabled={!canTake}
              title={!canTake ? `Maximum attempts (${currentQuiz.maxAttempts}) reached` : ''}
            >
      ...
      <Card>
        <CardContent className="pt-6 space-y-2">
          ...
          {attemptHistory && attemptHistory.length > 0 && (
            <p><strong>Your attempts:</strong> {attemptHistory.length} / {currentQuiz.maxAttempts}</p>
          )}
        </CardContent>
      </Card>
    </div>
