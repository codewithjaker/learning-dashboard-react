...
import { useParams, useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { ArrowLeft, Plus, GripVertical, Edit, Trash2, ChevronDown, ChevronRight, Video, FileText, HelpCircle, Code, PenTool, File } from 'lucide-react';
import { Button } from '../../components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '../../components/ui/card';
import { Input } from '../../components/ui/input';
import { Textarea } from '../../components/ui/textarea';
                    ...
                    {(items[section.id] || []).map((item, itemIdx) => (
                      <div key={item.id} className="flex items-center justify-between p-2 hover:bg-muted/50 rounded-lg transition-colors">
                        <div className="flex items-center gap-3">
                          {getItemIcon(item.type)}
                          <span className="text-sm">{itemIdx + 1}. {item.title}</span>
                          {item.isFree && <Badge variant="outline" className="text-xs">Preview</Badge>}
                          {item.duration && (
                        ...
                        <div className="flex gap-1">
                          {item.type === 'quiz' && (
                            <Button
                              variant="outline"
                              size="sm"
                              // onClick={() => navigate(`/quiz/manage/${item.id}`)}
                              onClick={() => navigate(`/quiz/manage/${quiz.id}`)}
                            >
                              <HelpCircle className="h-3 w-3 mr-1" /> Manage Quiz
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => openItemDialog(section.id, item)}>
                            <Edit className="h-3 w-3" />
